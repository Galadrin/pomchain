...
DOCKER_IMAGE_BUF  	  = pom/buf-cosmos:0.3.1
DOCKER_BUILDX_BUILDER     = pom-builder
...
LD_FLAGS  = \
    -X github.com/cosmos/cosmos-sdk/version.AppName=pomd      \
	-X github.com/cosmos/cosmos-sdk/version.Name=pomd         \
	-X github.com/cosmos/cosmos-sdk/version.ServerName=pomd   \
	-X github.com/cosmos/cosmos-sdk/version.ClientName=pomd   \
...
chain-init: ## Initialize the blockchain with default settings.
	...
	@rm -rf "${CHAIN_HOME}"; \
	${CHAIN_BINARY} init pom-node \
	  --chain-id=pom-${CHAIN} \
	  --home "${CHAIN_HOME}"; \
	\
	sed -i $(SED_FLAG) "s/\"stake\"/\"upom\"/g" "${CHAIN_HOME}/config/genesis.json"; \
	\
	...
	echo $$MNEMONIC_VALIDATOR \
	  | ${CHAIN_BINARY} keys add validator \
	      ...
	      --home "${CHAIN_HOME}"; \
	\
	${CHAIN_BINARY} add-genesis-account validator 1000000000upom \
	  --keyring-backend test \
	  --home "${CHAIN_HOME}"; \
	\
	NODE_ID=`${CHAIN_BINARY} tendermint show-node-id --home ${CHAIN_HOME}`; \
	${CHAIN_BINARY} gentx validator 1000000upom \
	  --node-id $$NODE_ID \
	  --chain-id=pom-${CHAIN} \
	  --keyring-backend test \
      --home "${CHAIN_HOME}"; \
...
chain-upgrade: build ## Test the chain upgrade from the given FROM_VERSION to the given TO_VERSION. You can pass also the proposal json file on PROPOSAL var
	...
	git clone -b ${FROM_VERSION} https://github.com/Galadrin/pomchain.git ${TARGET_FOLDER}/${FROM_VERSION}; \
	...
	if [[ ! -f "$$PROPOSAL" ]]; then \
        ...
        echo '{"messages": [{"@type": "/cosmos.upgrade.v1beta1.MsgSoftwareUpgrade","authority": "pom10d07y265gmmuvt4z0w9aw880jnsr700jh7kd2g","plan": {"name": "","time": "0001-01-01T00:00:00Z","height": "10","info": "","upgraded_client_state": null}}],"metadata": "ipfs://CID","deposit": "1upom"}' | \
	...
	sleep 10;\
 	$$BINARY_OLD tx gov submit-proposal $$PROPOSAL \
 		...
 		-b block; \
 	\
 	$$BINARY_OLD tx gov deposit 1 10000000upom \
     		--from validator \
     		--yes \
     		--home ${CHAIN_HOME} \
     		--chain-id pom-${CHAIN} \
...
proto-gen: proto-build ## Generate all the code from the Protobuf files
	...
	@cp -r https://github.com/Galadrin/pomchain/x/* x/
...
doc-command: ## Generate markdown documentation for the command
	...
	sed -i $(SED_FLAG) 's/(default \"\/.*\/\.pomd\")/(default \"\/home\/john\/\.pomd\")/g' $$OUT_FOLDER/*.md; \
	...
	sed -i $(SED_FLAG) 's/<appd>/pomd/g' $$OUT_FOLDER/*.md; \
...
$(RELEASE_TARGETS): ensure-buildx-builder
	@GOOS=$(word 3, $(subst -, ,$@)); \
    GOARCH=$(word 4, $(subst -, ,$@)); \
    BINARY_NAME="pomd-${VERSION}-$$GOOS-$$GOARCH"; \
	...
	docker rm -f tmp-pomd || true; \
	docker create -ti --name tmp-pomd $$BINARY_NAME; \
	docker cp tmp-pomd:/usr/bin/pomd ${RELEASE_FOLDER}/$$BINARY_NAME; \
	docker rm -f tmp-pomd; \
